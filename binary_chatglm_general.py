...
if __name__ == '__main__':
  ...
  arg_parser.add_argument("--btsize", type=int, default=2,
                          help="Please specify a model file to evaluate")
  arg_parser.add_argument("--rank_score", type=float, default=0.1,
                          help="Please specify a model file to evaluate")
  ...
  for n in range(n_epoch):
    ...
    for batch in tqdm.tqdm(trainloader):
      ...
      for i in range(len(batch['cur'])):
        doc_scores = bm25.get_scores(stem(batch['cur'][i]).split())

        BM25_top = torch.topk(torch.from_numpy(doc_scores).to(device), k=parsed_args.topk).indices
        all_bm25_top.append(BM25_top.unsqueeze(0))
      all_bm25_top = torch.cat(all_bm25_top,dim=0)
      presi_label = torch.zeros_like(simi_map).scatter_(1, all_bm25_top, 1).to(device)
      # print(torch.nonzero(presi_label))
      eps = 1e-8
      loss =  Loss(F.log_softmax(simi_map + eps, dim=1), F.softmax(presi_label, dim=1))
        # simi_score = F.softmax(simi_map[i])
        # #print(simi_score.shape, simi_map.shape,BM25_top)
        # for m in BM25_top:
        #   for n in range(len(simi_score)):
        #     if n not in BM25_top:
        #       loss += max(0, parsed_args.rank_score + simi_score[n] - simi_score[m])

      optimzer.zero_grad()
      loss.backward()
      optimzer.step()
      tr_loss += torch.abs_(loss).item()
      #print(torch.abs_(loss).item())
    ...
    torch.save(record, './binary_chatglm_rank_'+str(parsed_args.topk) + '_shot_record_' + str(parsed_args.dataset) + '_cand' + str(parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))
    if eval_metrics['ROUGE'] > bert_metric:
      bert_metric = eval_metrics['ROUGE']
      bert_metrics = eval_metrics
      torch.save(model, './binary_chatglm_rank_' + str(parsed_args.topk)+'_shot_model_' + str(parsed_args.dataset) + '_cand' + str(parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))
      torch.save(prediction, './binary_chatglm_rank_'+str(parsed_args.topk) + '_shot_prediction_' + str(parsed_args.dataset) + '_cand' + str(parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))

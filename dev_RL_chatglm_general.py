...
if __name__ == '__main__':
  ...
  for n in range(n_epoch):
    ...
    model.eval()
    for i in tqdm.tqdm(range(len(dev_questions))):

      q_emb = encode(model, dev_questions[i]).reshape(1,-1)
      #print(len(index))
      sample_candidcates = candidcate_questions
      tok_res = tokenizer(sample_candidcates, padding=True, return_tensors='pt')
      candidcate_encodings = encode_questions(model, tok_res['input_ids'].to(device), tok_res['attention_mask'].to(device))
      candidcate_norms = torch.norm(candidcate_encodings, dim=1)
      dev_norms = torch.norm(q_emb, dim=1)
      simi_map = torch.abs_(torch.mm(q_emb, candidcate_encodings.T)) / torch.mm(dev_norms.reshape(-1,1), candidcate_norms.reshape(1,-1))
      top = torch.topk(simi_map, k = parsed_args.topk, dim=-1).indices
      #print("top3: ", top3.shape)
      ...
      if pred == '':
        pred = 'hi'
      #print('i = ', i, 'pred = ', response.lower().split('\n')[0], 'gold = ', dev_data[i]['rewrite'])
      e.evaluate_metrics(cur_str=[dev_data[i]['cur']], restate_str=[dev_data[i]['rewrite']],
                         predict_str=[pred])
    ...
    torch.save(record, './dev_chatglm_RL_'+str(parsed_args.topk) + '_shot_record_' + str(parsed_args.dataset) + '_cand' + str(parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))
    if eval_metrics['ROUGE'] > bert_metric:
      bert_metric = eval_metrics['ROUGE']
      bert_metrics = eval_metrics
      torch.save(model, './dev_chatglm_RL_' + str(parsed_args.topk)+'_shot_model_' + str(parsed_args.dataset) + '_cand' + str(parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))

  # begin test
  model = torch.load('./dev_chatglm_RL_' + str(parsed_args.topk)+'_shot_model_' + str(parsed_args.dataset) + '_cand' + str(parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))
  model.eval()
  prediction = []
  for i in tqdm.tqdm(range(len(test_questions))):
    ...
    e.evaluate_metrics(cur_str=[test_data[i]['cur']], restate_str=[test_data[i]['rewrite']],
                       predict_str=[pred])
    prediction.append(
      {'context': test_data[i]['context'], 'cur': test_data[i]['cur'], 'restate': test_data[i]['rewrite'],
       'pred': pred})
  eval_metrics = e.get_metrics(reset=True)
  print("eval metrics on test = ", eval_metrics)
  torch.save(prediction,
             './dev_chatglm_RL_' + str(parsed_args.topk) + '_shot_prediction_' + str(parsed_args.dataset) + '_cand' + str(
               parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))

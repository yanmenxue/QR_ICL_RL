...
NUM_candidate = len(train_data) / 2

candidcate_questions = [train_data[i]['cur'] for i in range(NUM_candidate)]
test_questions = [test_data[i]['cur'] for i in range(len(test_data))]
c = range(NUM_candidate)

class trainset(Data.Dataset):
  def __init__(self):
    self.data = train_data[NUM_candidate:]

import tqdm
from transformers import BertTokenizer, BertModel, AutoTokenizer, AutoModel, AutoModelForSeq2SeqLM
import torch
import random
...
fast_model = AutoModelForSeq2SeqLM.from_pretrained("../pretrain_model/fast_chat").to(device)
fast_tokenizer = AutoTokenizer.from_pretrained("../pretrain_model/fast_chat")
fast_model = fast_model.eval()
...
for n in range(n_epoch):
  ...
  for batch in tqdm.tqdm(trainloader):
    ...
    for i in range(len(batch['cur'])):
      ...
      inputs = fast_tokenizer(prompt, return_tensors="pt", padding='max_length')
      outputs = fast_model.generate(input_ids=inputs['input_ids'].to(device), max_length=100)
      pred = ' '.join(fast_tokenizer.batch_decode(outputs, skip_special_tokens=True)[0].split()[1:])
  ...
  if eval_metrics['ROUGE'] > bert_metric:
    ...
    torch.save(model, 'RL_model')
    torch.save(prediction, './canard_data/fast_RL_five_shot_prediction_few')

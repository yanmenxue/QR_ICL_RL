...
if __name__ == '__main__':
  ...
  for n in range(n_epoch):
    ...
    torch.save(record, './chatglm_RL_'+str(parsed_args.topk) + '_shot_record_' + str(parsed_args.dataset) + '_cand' + str(parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))
    if eval_metrics['ROUGE'] > bert_metric:
      bert_metric = eval_metrics['ROUGE']
      bert_metrics = eval_metrics
      torch.save(model, './chatglm_RL_' + str(parsed_args.topk)+'_shot_model_' + str(parsed_args.dataset) + '_cand' + str(parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))
      torch.save(prediction, './chatglm_RL_'+str(parsed_args.topk) + '_shot_prediction_' + str(parsed_args.dataset) + '_cand' + str(parsed_args.num_cand) + '_train' + str(parsed_args.num_train) + '_seed' + str(parsed_args.seed))
